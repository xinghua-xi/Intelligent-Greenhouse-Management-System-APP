...
export const getGeminiDiagnosis = async (base64Image: string): Promise<DiagnosisResult> => {
  ...
  try {
    ...
    const prompt = `
      You are an expert agricultural agronomist. Analyze this image of a crop. 
      Identify the plant, diagnose any visible diseases, pests, or nutrient deficiencies.
      If the plant looks healthy, say so.
      Provide treatment recommendations if issues are found.
      
      Return the response in JSON format conforming to this schema:
      {
        "plantName": "string",
        "condition": "healthy" | "disease" | "pest" | "unknown",
        "diagnosis": "string (short description in Chinese)",
        "treatment": ["string (list of 2-3 actionable advice in Chinese)"],
...
const mockDiagnosis = (): DiagnosisResult => ({
  ...
  treatment: ["及时摘除病叶并带出田外销毁", "喷洒百菌清或代森锰锌进行防治", "控制棚内湿度，加强通风"],
  confidence: 88
});